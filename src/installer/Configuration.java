...
public class Configuration {
	 ...
	 */
	public Configuration(List<ConfigStage> stages, boolean isGUI) {
		this.list = stages;
		...
		try (BufferedReader b = new BufferedReader(new InputStreamReader(System.in))) {
			for (int i = 0; i < list.size(); i++) {
				...
				if(cur.getConfigManager() != null)
					cur.getConfigManager().managedConfigStages(cur, parameters);
				if (cur.isUsable()) {
					index = cur.getIndex();
					cur.run(b);
	...
	private void finishedStage(ConfigStage<?> cur) {
		if(gui)
